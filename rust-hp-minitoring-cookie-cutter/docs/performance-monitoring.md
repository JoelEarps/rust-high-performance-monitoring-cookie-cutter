...
1. **dhat** - Heap profiler for memory allocation tracking
2. **tokio-console** - Runtime monitoring and debugging for async Tokio applications
...
dhat is configured in `src/bin/testing.rs` with:
...
Criterion is configured in `benches/benchmark.rs`:
...
This project is organized as a Cargo workspace with convenient aliases defined in `.cargo/config.toml` at the workspace root. These aliases make it easy to run the testing binary with different monitoring configurations.
...
- **Workspace root**: Contains `Cargo.toml` with workspace configuration and `.cargo/config.toml` with aliases
- **Package**: `rust-hp-minitoring-cookie-cutter/` contains the actual library and binary code

All aliases automatically target the correct package in the workspace using the `-p rust-hp-minitoring-cookie-cutter` flag.
